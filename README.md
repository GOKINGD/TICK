...
- 支持 Skills、MCP、Hooks
- 本地长期记忆：只保存脱敏后的稳定偏好和习惯，可在 Settings 里查看和清空
...
运行时会在项目目录下生成 `chat-api-key`、`llm-settings.json`、`trace.jsonl`、`memory.json`、`tools/config.json` 等本地状态文件。这些文件可能包含密钥、trace 原始请求、脱敏后的长期记忆或本机路径，默认不会提交到 Git。
