...
struct ActivitySignal: Codable, Equatable {
    ...
    private var actionOutputContract: String {
        ...
        - 必须像一个主动式桌面 Agent：判断用户可能下一步要做什么，并给出可执行选项
        - 控制在 160 字以内，除非是错误诊断
        - 先给一句“我猜你正在...”或“我检测到...”
        - 给出 1-2 条关键证据或已提取信息
        ...
        ```
        - 按钮文案必须是动作，不要是“了解更多”；例如“校验本地 Docker 环境”“提取并准备执行命令”“对比本地报错”“生成补丁草案”
    ...
    var prompt: String {
        ...
        if type == "browser.page_idle" {
            ...
            return """
            用户正在浏览一个网页，并在页面上停留了超过 3 秒。这代表用户正在输入信息，不需要你复述页面。请预判用户可能下一步想做什么，并提供可执行行动选项。
            ...
            - Capture mode 为 metadata-only 时，只能基于标题和 URL 给轻量行动建议；无法判断就输出 TICK_SILENT
